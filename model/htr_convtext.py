...
class ResNetEmbed(nn.Module):
    """Pure-ResNet patch embed: same striding as MVP but without MobileViT blocks.
    Produces the same output shape (B, embed_dim, H', W') as MVP."""
    ...
    def __init__(self, nb_feat=384):
        ...
        self.layer1 = self._make_layer(
            mvp.BasicBlock, nb_feat // 4, 2, stride=(2, 1))
        self.layer2 = self._make_layer(
            mvp.BasicBlock, nb_feat // 2, 2, stride=2)
        self.layer3 = self._make_layer(mvp.BasicBlock, nb_feat, 2, stride=2)
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.maxpool(x)  # reuse same MaxPool2d (stateless)
        return x
...
class HTR_ConvText(nn.Module):
    ...
    def forward_features(self, x, use_masking=False, mask_ratio=0.5, max_span_length=8):
    ...
    def forward(self, x, use_masking=False, return_features=False, return_mask=False, mask_ratio=None, max_span_length=None):
        feats, masked_positions_1d = self.forward_features(
            x, use_masking=use_masking, mask_ratio=mask_ratio, max_span_length=max_span_length)
...
def create_model_resnet(nb_cls, img_size, mlp_ratio=4, **kwargs):
    """HTR_ConvText with a pure ResNet patch embed (no MobileViT blocks)."""
    model = HTR_ConvText(
        nb_cls,
        img_size=img_size,
        patch_size=(4, 64),
    ...
    model.patch_embed = ResNetEmbed(512)
    return model
